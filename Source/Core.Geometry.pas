...
uses
  System.Variants, System.Math, System.Classes,
  Core.Types, Core.Utils, Core.Arrays, Core.Messages, Core.Mesh, Core.Model;

type
  TGeometry = class
    class procedure InitObject(var AModel: TModel; AShapeType: TGX_ShapeType ; ASize: TVector3 ; AResolution: Integer );
...
class procedure TGeometry.InitObject(var AModel: TModel; AShapeType: TGX_ShapeType ; ASize: TVector3 ; AResolution: Integer );
begin
  case AShapeType of
...
begin
  ...
  AMesh := TMesh.Create('Cube Faces', ptTriangles, smFlat, [asPosition, asNormal]);
...
begin
  ...
  p_BaseMesh  := TMesh.Create('Base Mesh',  ptTriangles, smFlat, [asPosition, asNormal]);
  AModel.Meshes.Add(p_BaseMesh);
  p_CoverMesh := TMesh.Create('Cover Mesh', ptTriangles, smGouraud, [asPosition, asNormal]);
...
begin
  ...
  ALowerMesh := TMesh.Create('Lower Face', ptTriangles, smGouraud, [ asPosition, asNormal ]);
  AUpperMesh := TMesh.Create('Upper Face', ptTriangles, smGouraud, [ asPosition, asNormal ]);
  AOuterMesh := TMesh.Create('Outer Face', ptTriangles, smGouraud, [ asPosition, asNormal ]);
...
begin
  ...
  p_Mesh := TMesh.Create('Grid Lines', ptLines, smFlat, [ asPosition, asColor ]);
...
begin
  ...
  AMesh := TMesh.Create('Main Mesh', ptTriangles, smGouraud, [ asPosition, asNormal ]);
  AModel.Meshes.Add(AMesh);
...
begin
  ...
  AMesh := TMesh.Create('Spider Mesh', ptTriangles, smFlat, [ asPosition, asNormal ]);
  AModel.Meshes.Add(AMesh);
...
begin
  ...
  AMesh := TMesh.Create('Test Cross', ptLines, smFlat, [ asPosition ]);
  AModel.Meshes.Add(AMesh);
