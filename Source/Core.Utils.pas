...
function VarToSql(AVar: Variant): string;
...
begin
  ...
  case AVarType of
    varNull:
    	Result := 'NULL';
    varBoolean:
      if Boolean(AVar) = True then Result := IntToStr(SQL_TRUE) else Result := IntToStr(SQL_FALSE);
    varString, varOleStr, varUString:
      Result := QuotedStr(VarToStr(AVar));
    varByte, varWord, varLongWord, varSmallint, varInteger:
      Result := IntToStr(Integer(AVar));
    varDate:
      Result := '''' + FormatDateTime('yyyy-mm-dd hh:nn:ss', AVar) + '''';
    varSingle, varDouble, varCurrency, $111 {varTypeBCD DOUBLE}:
    begin
      i := Trunc(AVar);
      Result := IntToStr(i) + '.' + IntToStr(Round((AVar - i) * 100));
    end
    else
      Result := 'NULL';
  end;
