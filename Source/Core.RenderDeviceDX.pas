...
uses
  ...
  Core.Types, Core.Utils, Core.Arrays, Core.Mesh, Core.Model, unCamera, Core.Material,
  Core.RenderDevice, Core.ShaderDX,
  unCast;
...
type
  ...
  TGraphicsDeviceDX = class(TRenderDevice)
  private
    { directx }
    ...
    FVertexBuffers: TGX_NodeList<PGX_VertexBufferDX>;
    FIndexBuffers: TGX_NodeList<PGX_IndexBufferDX>;
    FResourceBuffers: TGX_NodeList<PGX_ResourceBufferDX>;
    procedure Clear();
...
constructor TGraphicsDeviceDX.Create( Windowed: Boolean ; AHandle: HWND ; AViewRect: TRect ) ;
begin
  inherited Create( Windowed, AHandle, AViewRect );

  FDeviceType := dtDirectX;

  FVertexBuffers    := TGX_NodeList<PGX_VertexBufferDX>.Create();
  FIndexBuffers     := TGX_NodeList<PGX_IndexBufferDX>.Create();
  FResourceBuffers  := TGX_NodeList<PGX_ResourceBufferDX>.Create();

  Start();
end;

destructor TGraphicsDeviceDX.Destroy;
begin
  Clear();
...
procedure TGraphicsDeviceDX.ClearScene();
var
...
procedure TGraphicsDeviceDX.CreateDeviceAndSwapChain();
var
...
begin
  ...
  SwapChainDesc.Windowed := TgmDeviceFlags.gmWindowed in FDeviceFlags;
  ...
  if FAILED(HR) then
    raise Exception.Create('TGraphicsDeviceDX'#13 + 'Error ' + IntToHex(hr, 8) + ': Cannot create Device and Swap Chain');
end;

procedure TGraphicsDeviceDX.CreateRenderTargetAndDepthStencil();
var
  hr: HRESULT;
...
procedure TGraphicsDeviceDX.CreateViewPorts();
var
...
procedure TGraphicsDeviceDX.CreateRasterizers();
var
...
begin
  ...
  FRasterState[ Integer( TGX_RasterMode.rmDefault )]    := CR( D3D11_FILL_SOLID,     D3D11_CULL_NONE, False, False, True,  0 );
  FRasterState[ Integer( TGX_RasterMode.rmWireframe )]  := CR( D3D11_FILL_WIREFRAME, D3D11_CULL_BACK, False, False, False, Round( 0.00001 / ( 1 / Power( 2, 23 ))));
end;

procedure TGraphicsDeviceDX.CreateBlendStates();
var
  hr: HRESULT;
...
procedure TGraphicsDeviceDX.CreateDepthStencilStates();
var
  hr: HRESULT;
...
procedure TGraphicsDeviceDX.CreateShaders();
begin
  ...
  FMainBufferID := Getbuffer(SizeOf(TGX_MainConst), nil, Cardinal(D3D11_BIND_CONSTANT_BUFFER));
  FCastBufferID := GetBuffer(SizeOf(TGX_CastConst), nil, Cardinal(D3D11_BIND_CONSTANT_BUFFER));
end;

procedure TGraphicsDeviceDX.CreateSampler;
var
  hr: HRESULT;
...
begin
  ...
  if FAILED(HR) then
    raise Exception.Create('TGraphicsDeviceDX'#13 + 'Could not create Sampler State');
end;

procedure TGraphicsDeviceDX.Start();
begin
  FError := False;
...
procedure TGraphicsDeviceDX.Clear();
var
  i: Integer;
begin
  FActive := False;
...
procedure TGraphicsDeviceDX.SetViewRect( AViewRect: TRect );
var
  hr: HRESULT;
begin
  FViewRect := AViewRect;
...
function TGraphicsDeviceDX.GetBuffer( ByteWidth: UINT; InitSys: Pointer; BindFlags: UINT ): ID3D11Buffer;
var
...
procedure TGraphicsDeviceDX.SetRasterMode( ARasterMode: TGX_RasterMode );
begin
...
procedure TGraphicsDeviceDX.SetBlendState( ABlendState: TGX_BlendState );
var
...
procedure TGraphicsDeviceDX.SetFullScreen( GoFullScreen: Boolean );
begin
  FSwapChain.SetFullscreenState( GoFullScreen, nil );
end;

function TGraphicsDeviceDX.CreateVertexBuffer( AVertexArray: TGX_Array ): Integer;
var
...
procedure TGraphicsDeviceDX.UpdateVertexBuffer( AVBufferIndex: Integer ; AVertexArray: TGX_Array );
var
  AVertexBuffer: PGX_VertexBufferDX;
begin
...
function TGraphicsDeviceDX.CreateIndexBuffer( AIndexArray: TGX_Array ): Integer;
var
...
function TGraphicsDeviceDX.CreateResourceBuffer( ATexture: TGX_Texture ): Integer;
var
...
function TGraphicsDeviceDX.GetShaderIndex( var ASemantics: TGX_Semantics ; ATexture: TGX_Texture ): Integer;
var
  i: Integer;
begin
...
procedure TGraphicsDeviceDX.UpdateMainBuffer( AShaderIndex: Integer );
begin
...
procedure TGraphicsDeviceDX.UpdateCastBuffer( AShaderIndex: Integer );
begin
...
// constant buffer.
//********************************************************************************************************************//
procedure TGraphicsDeviceDX.SetShaders( AShaderIndex: Integer ; ABufferIndex: Integer );
var
  AShader: TShaderDX;
begin
...
procedure TGraphicsDeviceDX.SetResources( ATexture: TGX_Texture );
begin
  ...
  begin
    ...
    if(( ShaderMode = smTest ) and ( FTestPattern.RBufferIndex >= 0 )) then
      IContext.PSSetShaderResources( 0, 1, @FResourceBuffers[ FTestPattern.RBufferIndex ].BufferPointer )
    else
...
procedure TGraphicsDeviceDX.SetTopology( ATopology: TVertexTopology );
begin
  case ATopology of
...
procedure TGraphicsDeviceDX.Draw(ACount: Integer; AVBufferIndex, AIBufferIndex: Integer; Instances: TInstanceArray);
var
...
procedure TGraphicsDeviceDX.GetDeviceInfo();
begin
  //
end;

procedure TGraphicsDeviceDX.Show();
begin
