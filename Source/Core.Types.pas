...
const
  ...
  dxZeroRect: TRect = (Left: 0; Top: 0; Right: 0; Bottom: 0);
  dxLightGreen: TColor = $80FF80;
  dxLightRed: TColor = $C0C0FF;
  dxLightBlue: TColor = $FFC0C0;
  dxLightYellow: TColor = $40FFFF;
  dxLightGray: TColor = $C0C0C0;
  dxLightOrange: TColor = $60C0FF;
  dxLightOlive: TColor = $40E0C0;
  dxLightWhite: TColor = $F0F0F0;
  ////////////////////////////////////////////////////////////

type
  TGX_Status = (stCreated = 0, stLoaded, stModified);
  TGX_RasterMode = (rmDefault, rmWireframe);
  TGX_ShaderMode = (smBlank, smTest, smColor);
  TGX_ViewMode = ( vmCamera, vmScreen);
  TGX_ShadeMode = ( smFlat, smGouraud);
  TVertexTopology = (ptPoints, ptLines, ptTriangles, ptTriangleStrip);
  TGX_ProjMode = (pmScreen, pmCamera);
  TGX_Semantic = (asUnknown, asIndex, asCounter, asPosition, asNormal, asTexcoord, asColor, asTerrain, asBoneIndex, asBoneWeight);
  TGX_Semantics = set of TGX_Semantic;
  TShaderSemantics = set of TGX_Semantic;

  TDX_SelectEvent = procedure(Sender: TObject) of object;
  TDX_FunctionInfo = (fiCount, fiCaption, fiEvent);
  TDX_DataStatus = (dsCreated, dsLoaded, dsModified, dsDeleted);
  TdxDataStatus = TDX_DataStatus;
  TDX_RecordStatus = (rsCreated, rsOpen, rsClosed, rsCanceled, rsDeleted, rsArchived );
  TDX_NodeType = (ntNode, ntFolder, ntHeader, ntFooter);
  TDX_ViewStatus = set of TDX_RecordStatus;
  TDX_RectState = (rsClear, rsInactive, rsHot, rsSelected );
  TDateOrder = (doMDY, doDMY, doYMD );
  TDX_SortStatus = (stNone, stUp, stDown );
  TDX_FontType = (ftTitle, ftSubTitle, ftStandard, ftStandardLight, ftSmall, ftSmallLight, ftMini, ftLabel);
  ...
  TParameter = record
    ...
    Caption: string;
    Value: Variant;
    constructor Create(ID: Integer; Caption: string; Value: Variant);
  end;
  ...
  function dxVarToStr(value: Variant): string;
...
constructor TParameter.Create(ID: Integer; Caption: string; Value: Variant);
begin
  Self.ID := ID;
